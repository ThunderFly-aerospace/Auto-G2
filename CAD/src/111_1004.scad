...
module triangle(){
    ...
    edge_distance = 15;
    spacer_disc_diameter = M4_screw_diameter + 2;
            ...
            difference() {
                union(){
                 cylinder(r = 3+9, h = thickness, center = true, $fn = 100);
                    translate([0,0,-thickness/2])cylinder(d = spacer_disc_diameter, h = thickness + 0.5, $fn = 100);
                }
                 cylinder(d = M4_screw_diameter, h = 3* thickness, center = true, $fn = 20);
                
               

        for (i = [1:rotor_blades_count]){
                rotate([0, 0, i*angle_between_blades + angle_between_blades/2 + 180]) translate([0, edge_distance, 0])cube([30, 15, 10], center = true);
